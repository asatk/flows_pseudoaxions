...
model, distribution, made_list = flowmodel.compile_MAF_model(num_made, num_inputs=2, num_cond_inputs=num_cond_inputs, return_layer_list=True)
...
gaus_covs = myutils.cov_change_none(gaus_pts, gaus_cov_indv)
...
if conditioned:
    data = [gaussians, gaussian_labels]
else:
    data = gaussians
...
label = np.array([1./(n_gaus_train + 1)])
# label = np.array([3./(ngausx + 1), 3./(ngausy + 1)])
...
samples = np.array(distribution.sample((n_samples, ), bijector_kwargs=current_kwargs))
...
cov_exp = myutils.cov_change_none(np.array([[label,0.5]]), gaus_cov_indv)
print("\nExpected Stats: \n - Mean: ", mean_exp, "\n - Cov: ", cov_exp)
...
# plt.scatter(labels[:, 0], labels[:, 1], s=25, color='red')
plt.scatter(labels, np.ones(n_gaus_train) * yval, s=25, color='red')
# plt.scatter(label[0], label[1], s=25, color='yellow')
